...
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
# GREEN = (50, 168, 82) # Não será mais a cor de fundo principal se tivermos grama
RED = (200, 0, 0)
DARK_RED = (100, 0, 0)
SKIN = (255, 220, 200)
PURPLE = (138, 43, 226)
...
# Configurações do Jogo (sem alterações)
...
ASSET_PATH = "assets" # Caminho base para assets
SPRITE_PATH = os.path.join(ASSET_PATH, "sprites")
# TILE_PATH = os.path.join(ASSET_PATH, "tiles") # Se quiser organizar tiles separadamente

# --- Tamanhos dos sprites ---
VAMPIRE_SIZE = (60, 60)
HUMAN_SIZE = (40, 40)
ENEMY_SIZE = (100, 100)
PROJECTILE_SIZE_NORMAL = (20, 20)
PROJECTILE_SIZE_BLOODBOMB = (30, 30)
PROJECTILE_SIZE_NIGHTBEAM = (100, 18)

# === ASSETS GLOBAIS ===
SPRITES = {}
GRASS_TILE_IMG = None # Variável para armazenar a imagem da grama

def load_image(filepath, size=None, use_alpha=True):
    """Função auxiliar para carregar e opcionalmente redimensionar imagens."""
    try:
        ...
        if size: # Cria um placeholder se o tamanho for especificado
            fallback_surface = pygame.Surface(size, pygame.SRCALPHA)
            fallback_surface.fill((255, 0, 255, 100)) # Magenta
            pygame.draw.rect(fallback_surface, WHITE, fallback_surface.get_rect(), 1)
            return fallback_surface
        return None # Retorna None se não puder criar placeholder (ex: tile sem tamanho definido)

def create_sprites_and_tiles():
    global GRASS_TILE_IMG # Para modificar a variável global
    ...
    SPRITES['enemy'] = load_image(os.path.join(SPRITE_PATH, "enemy.png"), ENEMY_SIZE)
    ...
    # Fallbacks (se o carregamento da imagem falhar e retornou placeholder)
    if SPRITES['vampire'] and SPRITES['vampire'].get_at((0,0))[:3] == (255,0,255):
    ...
    # Adicione fallbacks similares para outros sprites se desejar...

    # Carregar Tile de Grama
    # Suponha que sua imagem de grama seja 'grass_tile.png' e esteja em SPRITE_PATH
    # Você pode ajustar o nome do arquivo e o caminho.
    # Não vamos redimensionar o tile aqui, usaremos seu tamanho original.
    GRASS_TILE_IMG = load_image(os.path.join(SPRITE_PATH, "grass_tile.png"), use_alpha=False) # use_alpha=False se for JPG, True se PNG
    if GRASS_TILE_IMG is None:
        print("Falha ao carregar grass_tile.png. Fundo será uma cor sólida.")


# === CLASSES ===
# (Nenhuma alteração nas classes GameObject, Vampire, Human, Enemy, Projectile,
# BloodBombExplosion, BloodBombProjectile)
class GameObject(pygame.sprite.Sprite):
    def __init__(self, image_key, pos):
        ...
        else: # Fallback para um sprite simples se a chave não existir ou a imagem for None
            print(f"Aviso: Chave de sprite '{image_key}' não encontrada ou imagem nula. Usando placeholder.")
            self.image = pygame.Surface((30,30), pygame.SRCALPHA)
            self.image.fill(PURPLE) # Cor de placeholder
...
class Human(GameObject):
    ...
    def get_sucked(self):
        if not self.sucked:
            self.sucked = True
            self.image = SPRITES.get('human_dead', self.image) # Fallback se 'human_dead' não carregar
...
class BloodBombExplosion(GameObject):
    def __init__(self, pos):
        # Usaremos uma chave placeholder para GameObject, mas a imagem será sobrescrita
        super().__init__('__blood_explosion_placeholder__', pos) # Chave não precisa estar em SPRITES
        self.image = pygame.Surface((80, 80), pygame.SRCALPHA)
        pygame.draw.circle(self.image, ORANGE, (40, 40), 40)
        pygame.draw.circle(self.image, RED, (40, 40), 30)
        self.rect = self.image.get_rect(center=pos) # Recalcula o rect
        self.damage = 30
        self.spawn_time = pygame.time.get_ticks()
        self.duration = 300
...
class GameState:
    ...
    def start_new_stage(self):
        ...
        for _ in range(num_enemies):
            ...
            # Certifique-se que ENEMY_SIZE está definido ou use o tamanho do sprite carregado
            enemy_sprite_size = SPRITES.get('enemy', pygame.Surface((30,30))).get_size()
            ...
            if GRASS_TILE_IMG:
                ...
                for x in range(0, WIDTH, tile_w):
                    for y in range(0, HEIGHT, tile_h):
                        screen.blit(GRASS_TILE_IMG, (x, y))
            else:
                screen.fill((50, 168, 82)) # Cor verde sólida como fallback
        elif self.current_map == "castle":
            screen.fill(BLACK)
        else:
            screen.fill(GRAY)

        self.all_sprites.draw(screen) # Desenha todos os sprites (vampiro, inimigos, etc.) sobre o fundo
        self.draw_hud()
...
if __name__ == '__main__':
    ...
    create_sprites_and_tiles() # Carrega os sprites e o tile de grama
