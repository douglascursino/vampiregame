    ...
    def handle_input(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Mudança de Mapa - Adicionado a verificação do mapa atual
                if event.key == pygame.K_F1:
                    if self.current_map != "castle":
                        self.change_map("castle")
                elif event.key == pygame.K_F2:
                    if self.current_map != "hunt_forest":
                        self.change_map("hunt_forest")
                elif event.key == pygame.K_F3:
                    if self.current_map != "hunt_village":
                        self.change_map("hunt_village")
